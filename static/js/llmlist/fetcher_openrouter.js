 ...
 * `type`, `code`, `isTimeout` e `userMessage`, così il chiamante può
...
/** Timeout massimo della richiesta di discovery, in millisecondi. */
const REQUEST_TIMEOUT_MS = 15000;

/** Codice interno per gli errori non legati a una risposta HTTP. */
const NO_HTTP_CODE = 0;

// ============================================================================
// TIPI DI ERRORE CLASSIFICATI
// ============================================================================

const ERR_MISSING_KEY = "MissingKeyError";
const ERR_INVALID_KEY = "InvalidKeyError";
const ERR_PAYMENT = "PaymentRequiredError";
const ERR_RATE_LIMIT = "RateLimitError";
const ERR_SERVER = "ServerError";
const ERR_HTTP = "HttpError";
const ERR_TIMEOUT = "TimeoutError";
const ERR_NETWORK = "NetworkError";
const ERR_PARSE = "ParseError";
 ...
 *
 * @param {string} type - Tipo di errore (es. TimeoutError).
 ...
 */
const _createModelError = function(type, message, code, userMessage) {
...
export const fetchOpenRouterModels = async function(apiKey) {
    ...
    if (!apiKey || apiKey.trim() === "") {
        const missingErr = _createModelError(
            ERR_MISSING_KEY,
    ...
    try {
        ...
        if (error.name === "AbortError") {
            ...
            const timeoutErr = _createModelError(
                ERR_TIMEOUT,
                timeoutMsg,
                408,
        ...
        const networkErr = _createModelError(
            ERR_NETWORK,
            netMsg,
            NO_HTTP_CODE,
    ...
    try {
        ...
        const jsonErr = _createModelError(
            ERR_PARSE,
            jsonMsg,
            NO_HTTP_CODE,
    ...
    if (!rawModels) {
        const payloadErr = _createModelError(
            ERR_PARSE,
